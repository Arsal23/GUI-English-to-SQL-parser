...
background=Label(window, text='SQL Coverter', bg = base, font=(bold, 20)).pack(pady = 15)

row1 = Frame(window, bg=base)
input = StringVar(row1)

Label(row1, text='Enter a regular statement: ', bg=base, font=(bold, 12)).pack(side='top', padx=10)
Entry(row1, textvariable=input, bg=white, width=45, font=bold).pack()
row1.pack()

Button(window, text="Parse Sentence", font=(bold, 12), command=parse).pack(pady=10)

row2 = Frame(window, bg=base)
sql = StringVar(row2)
sql.set("")
Label(row2, text="Generated SQL statement: ", bg=base, font=(bold, 12)).pack(side='top', padx=10)
Label(row2, textvariable=sql, bg=white, font=(bold, 12), width=50).pack(side='right', padx=10)
row2.pack()

row3 = Frame(window, bg=base)
Label(row3, bg=base, font=(bold, 12)).pack(side='left', padx=10)
Button(row3, text="SQL Command", font=(bold, 12), command=execute).pack(side='right', padx=10)
row3.pack(pady=10)

output = StringVar(window)
Label(window, textvariable=output, width=55, height=20, bg=white, border=10, anchor='nw', padx=10, pady=10, font=(bold, 14)).pack()
